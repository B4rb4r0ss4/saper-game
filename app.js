  ...
  if (current.length !== 0) {
    current.forEach(element => {
      ...
      if (el[element].bombsAround === 0) {
        ...
        document
          .getElementById(el[element].index)
          .classList.remove('blueField');

        el[element].freeAround.forEach(elements => {
          el[elements].active === true && newCurrent.push(elements);
        });
      } else if (
        el[element].freeAround.some(isSthAround) === true &&
    ...
    const newArray = [...new Set(newCurrent)];
    return openFreeFields(el, newArray, fieldHeight);
  }
};
...
class Game {
    ...
    this.text = `<div class="container" oncontextmenu="return false">
        ...
        <div class="parameters-container">
            <section class="parameters">
                ...
                <input type="number" class="board-width inp" placeholder="szerokość max.40" min="5" max="40" value="${
                  this.boardWidth
                }"> <span class="x">x</span>
                <input type="number" class="board-height inp" placeholder="wysokość max.40" min="5" max="40" value="${
                  this.boardHeight
                }">
            </section>
            ...
            <section class="parameters">
                ...
                <input type="number" class="bombs inp" placeholder="bomby" min="5" value="${
                  this.howManyBombs
                }">
                <label class="checkbox-container">
                    <input type="checkbox" class="checkbox" id="open-move" name="Opening-move" ${
                      this.openMove ? 'checked="true"' : ''
                    }">
  ...
  drawBoard() {
    ...
    for (let i = 0; i < this.boardHeight; i++) {
      ...
      for (let j = 0; j < this.boardWidth; j++) {
        const html = `<div style="height: ${this.fieldHeight}px; font-size:${
          this.fieldHeight / 2.1875
        }px" class="field blueField" id=${id}></div>`;
        document
          .getElementById(`${i}-row`)
    ...
    if (this.whichTurn === 0) {
      ...
      while (howManyBombsLeft > 0) {
        const random = Math.floor(
          Math.random() * (this.boardWidth * this.boardHeight)
        );
        ...
        if (this.openMove) {
          boardElements[idEl].findFunctions.forEach(element => {
            if (boardElements.findIndex(element) !== -1) {
              if (
                boardElements[boardElements.findIndex(element)].isBomb === true
              ) {
      ...
      this.boardElements[idEl].displayYourself(
        this.boardHeight,
        this.boardElements
      );
      if (this.boardElements[idEl].bombsAround === 0)
        openFreeFields(
          this.boardElements,
          this.boardElements[idEl].freeAround,
          this.fieldHeight
        );
  ...
  updateFlag = () => {
    document.querySelector(
      '.bomb-left'
    ).innerHTML = `<img src="flag.png" class="flag" alt="flaga">${this.flagNumber}/${this.howManyBombs}`;
  };
  ...
  timer = () => {
    if (this.result === 'inProgress') {
      ...
      document.querySelector('.timer').textContent = `Minęło: ${
        zeroBeforeHours + this.hours
      }.${zeroBeforeMinutes + this.minutes}.${zeroBeforeSeconds + this.second}`;
    }
  };
}
...
class boardElement {
  ...
  displayYourself(fieldHeight, el) {
    if (this.isBomb === true && this.active === true) {
      document.getElementById(
        ...
        fieldHeight / 1.3 - 4
      }px; height: ${fieldHeight / 1.3 - 4}px" class="bomb" alt="bomba">`;
      document.getElementById(this.index).classList.toggle('blueField');
      this.active = 'lose';
    } else if (this.isBomb === false) {
      document
        .getElementById(`${this.index}`)
        .classList.add(`Bombs${this.bombsAround}`);
      document.getElementById(
        `${this.index}`
      ).innerHTML = `<p class="text">${this.bombsAround}</p>`;
      document.getElementById(this.index).classList.toggle('blueField');
      this.active = false;
    }
  }
  ...
  displayFlag(fieldHeight, el) {
    document.getElementById(
      this.index
    ).innerHTML = `<img src="flag.png" style="width: ${
      fieldHeight - 7
    }px; height: ${fieldHeight - 7}px" class="flag-field" alt="flaga">`;
...
const newGame = () => {
  ...
  if (checkInputs(inputsValues)) {
    ...
    document
      ...
      .insertAdjacentHTML(
        'afterbegin',
        `<output class="bomb-left"><img src="flag.png" class="flag" alt="flaga"> ${inputsValues.howManyBombs}/${inputsValues.howManyBombs}</output><output class="timer">Minęło: 00.00.00</output>`
      );
    game.drawBoard();
